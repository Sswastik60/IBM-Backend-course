...
//Need  to use postman to test the endpoints. Use the following endpoints to test the application:
//1. Register a new user: POST http://localhost:5000/register with JSON body { "username": "your_username", "password": "your_password" }
//2. Login with the registered user: POST http://localhost:5000/login with JSON body { "username": "your_username", "password": "your_password" }
//3. Access the authenticated endpoint: GET http://localhost:5000/auth/get_message (after logging in)